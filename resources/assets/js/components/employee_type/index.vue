<template>
    <v-card>
        ...
        <v-card-text>
             <vue-bootstrap4-table :rows="employee_types" :columns="columns" :config="config" >
        ...
        <edit-type :dialog="dialog" :employee_type="employee_type" @close="close"  @employee_type="update"></edit-type>
...
export default {
    ...
    methods:{
        search(){
            axios.get('/api/auth/employee_type')
                 .then((response)=>{
                    ...
                    this.employee_types = response.data.types;
        ...
        edit (item) {
            ...
            axios.get(`/api/auth/employee_type/${item.id}/edit`)
            .then(response => {
                this.employee_type = response.data
            })
            .catch(error => {
                console.log(error);
            });
        ...
        update (item) {
            console.log(item);
            axios.post('/api/auth/employee_type', item)
                  .then(response => {
        ...
        destroy (item) {
            ...
            axios.delete(`/api/auth/employee_type/${item.id}`)
            .then((response) =>{
                // success_delete = true;
                this.search();
