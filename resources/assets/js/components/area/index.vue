<template>
    <v-card>
        ...
        <v-card-text>
             <vue-bootstrap4-table :rows="areas" :columns="columns" :config="config" >
        ...
        <edit-area :dialog="dialog" :area="area" @close="close"  @area="update"></edit-area>
...
export default {
    ...
    methods:{
        ...
        edit (item) {
            ...
            axios.get(`/api/auth/area/${item.id}/edit`)
            .then(response => {
        ...
        destroy (item) {
            ...
            axios.delete(`/api/auth/area/${item.id}`)
            .then((response) =>{
                // success_delete = true;
                this.search();
