...
<v-btn @click="create();" color="primary" dark class="mb-2">Nuevo</v-btn>    
        ...
        <template slot="expand" slot-scope="props">
            <v-card flat v-if="specie">
                <v-card-text>{{ specie.name }}</v-card-text>
                <v-card-text>{{ specie.description }}</v-card-text>
            </v-card>
        </template>
...
export default {
    ...
    mounted()
    {
        this.getSpecies().then(
            this.getDataFromApi()
                .then(data => {
                this.species = data.species
                this.totalSpecies = data.total
                })
        );                
    },
    methods:{
        ...
        getSpecies(withFilter){
            console.log("starting receiving data");
            return new Promise((resolve,reject)=>{
               this.loading = true
               let filterName = withFilter==true?this.filterName:'name';
               console.log(withFilter);
               axios.get('/api/auth/getSpecieData',{ name:filterName,value:this.filterValue})
                    .then((response) => {
        ...
        getResult(withFilter){
            ...
            //axios.post('/api/providers/getdata',{name:this.filterName})
              ...
              this.getSpecies(withFilter).then(
                        this.getDataFromApi()
                            .then(data => {
                            this.species = data.species
                            this.totalSpecies = data.total
